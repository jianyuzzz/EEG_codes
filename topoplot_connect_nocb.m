function [] = topoplot_connect_nocb(displayStruct, loc_file)
...
cM = colormap;
if ~isfield(displayStruct, 'connectStrength')% if there's no cS, 32*ones(N,1)
    ...
    end % extract max and min of the data
    xp = displayStruct.connectStrengthLimits(1);% min
    yp = displayStruct.connectStrengthLimits(2);% max
    ...
    if xp == yp
        cmapPos = ceil(size(cM, 1)/2)*ones(size(displayStruct.chanPairs, 1), 1);
    else% find the corresponding colormap to the given cStrength
